...
//   x86_64-w64-mingw32-g++ -O2 -DUNICODE -D_UNICODE lowcast.cpp -o LowCast.exe
//       -mwindows -static -lws2_32 -lole32 -lcomctl32 -lgdi32 -luser32
// ============================================================================
...
// ---------------------------------------------------------------------------
static const size_t MAX_CLIENT_BACKLOG = 4 * 1024 * 1024;  // ~10s @48k/16 — stalled client
...
static void fanout(const uint8_t* data, size_t len) {
    ...
    for (Client* c : G.clients) {
        if (c->dead) continue;
        if (c->q.size() + len > MAX_CLIENT_BACKLOG) { c->dead = true; continue; }
...
static DWORD WINAPI capture_thread(LPVOID) {
    ...
    if (FAILED(hr)) { ui_log(L"[audio] device enumerator failed"); return 1; }
    ...
    if (FAILED(hr)) { ui_log(L"[audio] mix format failed"); return 1; }
    ...
    G.sample_rate.store(wf->nSamplesPerSec * (uint32_t)G.upmult.load());
    ...
    if (FAILED(hr)) { ui_log(L"[audio] WASAPI loopback init failed"); return 1; }
    ...
    while (G.capture_run.load()) {
        ...
        {
            static uint64_t meas_t0 = 0, meas_frames0 = 0;
            static uint64_t real_frame_count = 0;
...
static bool send_chunk(SOCKET s, const uint8_t* data, size_t len) {
    char hdr[16];
    int hl = snprintf(hdr, sizeof(hdr), "%zx\r\n", len);
    if (!send_all(s, hdr, hl)) return false;
    if (!send_all(s, data, (int)len)) return false;
    return send_all(s, "\r\n", 2);
}
...
static DWORD WINAPI client_thread(LPVOID param) {
    ...
    // pump: grab whatever is queued, ship it immediately
    ...
    while (ok) {
        ...
        if (c->dead) ok = false;
            ...
            local.assign(c->q.begin(), c->q.end());
            c->q.clear();
        }
        LeaveCriticalSection(&G.cs);
        if (!ok) break;
        if (local.empty()) { Sleep(2); continue; }
        ok = send_chunk(s, local.data(), local.size());
        if (ok) c->sent_bytes += local.size();
    }
    send_all(s, "0\r\n\r\n", 5);
    closesocket(s);
...
static LRESULT CALLBACK wndproc(HWND h, UINT m, WPARAM wp, LPARAM lp) {
    ...
    case WM_COMMAND: {
        ...
        if (id == 3 && HIWORD(wp) == BN_CLICKED) {           // rescan
            ...
            G.beep_on.store(on);
            ui_log(on ? L"[test] metronome ON — 1 kHz tick each second; latency = flash\x2192sound gap"
            ...
            static const int lats[7] = { 25, 50, 75, 100, 150, 250, 350 };
            int sel = (int)SendMessageW(G.cmb_aplat, CB_GETCURSEL, 0, 0);
            if (sel >= 0 && sel < 7) G.ap_latency_ms.store(lats[sel]);
            ui_log(L"[raop] buffer changed  14 takes effect on next AirPlay START");
            ...
            EnterCriticalSection(&G.cs);
            for (Client* c : G.clients) c->dead = true;
            LeaveCriticalSection(&G.cs);
            ...
            G.fmt.store(sel);
            EnterCriticalSection(&G.cs);
            for (Client* c : G.clients) c->dead = true;   // old-format streams end
            LeaveCriticalSection(&G.cs);
...
struct RtspConn {
    ...
    bool open(const std::string& host, int port) {
        ...
        char idbuf[24]; snprintf(idbuf, sizeof(idbuf), "%08X%08X", rand() * rand(), rand() * rand());
        cid = idbuf;
        return true;
    }
    ...
    std::string request(const char* method, const std::string& ruri,
        ...
        if (!send_all(s, req.data(), (int)req.size())) { if (ok_out) *ok_out = false; return {}; }
...
static void qos_tag_voice(SOCKET s, const sockaddr_in& dst) {
    ...
    if (qh && pAdd) {
        QOS_FLOWID fid = 0;
        pAdd(qh, s, (sockaddr*)&dst, QOSTrafficTypeVoice_, QOS_NON_ADAPTIVE_FLOW_, &fid);
    }
}
...
static bool raop_handshake() {
    ...
    char sid[32]; snprintf(sid, sizeof(sid), "%u", (unsigned)(rand() * 2654435761u));
    ...
    RA.seq = (uint16_t)rand();
    RA.rtptime = (uint32_t)rand();
    RA.ssrc = (uint32_t)(rand() * 2654435761u);
    char rihdr[128];
...
static DWORD WINAPI raop_stream_thread(LPVOID) {
    SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_TIME_CRITICAL);
    if (!raop_handshake()) { RA.run = false; return 1; }
    RtspConn& c = RA.rtsp;
...
static void raop_stop() {
    if (!RA.run.load()) return;
    RA.run.store(false);
    if (RA.thread) { WaitForSingleObject(RA.thread, 3000); CloseHandle(RA.thread); RA.thread = nullptr; }
}
